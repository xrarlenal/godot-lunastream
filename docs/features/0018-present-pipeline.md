...
| 项 | 值 |
|---|---|
| 状态 | **进行中**：着色器、推送常量与管线代码已写好；**引擎内的 GLSL→SPIR-V 没通过**，自检暂未接入判据 |
| 阶段 | P0 |
...
- 呈现只支持"两块平面"（`luma_chroma`）。单块打包纹理（`interleaved_single`）
  **试过、已撤回**：着色器采样（亮度占上 2/3、色度占下 1/3）与绑定 3 都写完了，
  但读回**全黑**；它没有生产者（平台导入器一律给两块纹理），加"回读输入纹理"来区分
  「上传失败」与「采样错位」时，Metal 驱动还会 ABRT。没有可对照的参照物、又无法在
  合理代价内定位，就按项目规则撤回到**明确报错**（`UnsupportedPlaneSet`）——
  留一个"不确定能不能跑"的分支比少一条分支糟得多。要重启这条分支，先给它找一个
  真实生产者（例如某个平台导入器改为单纹理），那时的对照物会自己出现。
