...
# 续：用 `--verbose` 量化（2026-09-25）

方案 A 已经落地（我们自己的类走"创建期引用延迟交还" + 销毁时无守卫的
`Object.destroy()`）。现在把**残余**量化清楚，而不是笼统说"还在漏"。

## 实测数字

让引擎逐个列出残留对象（`Leaked instance: <类名>:<id> - Reference count: <n>`）：

| 场景 | 残留数 | 都是什么 |
|---|---|---|
| 0003 的 headless 自检（完全不碰 RD） | **2** | 一个**空类名**的实例——gdzig 的类注册机制带来的（0003 时就有，源工程同样中招） |
| 导入器自检（大量纹理/管线操作） | **28** | 绝大多数是 RD 辅助对象：`RDTextureFormat` / `RDTextureView` / `RDUniform` / `RDShaderSource` / `RDShaderSPIRV` / `RDSamplerState` |
| 播放烟测（一路流、约 10 帧） | **11** | 同上，量更小 |

## 一次决定性实验（推翻了我原本的假设）

原本的判断是"每建一个辅助对象就漏一个"。**用实验否掉了**：在自检里额外做 4 次
`RdTextureView.init()` + `unreference()`，退出时的残留数**纹丝不动**（11 → 11）。

结论：**gdzig 的 `init()` + `unreference()` 本身不漏**。残留集中在少数具体调用点——
最可能是"描述符对象被交给引擎创建 RID、而 RID 的释放发生在退出路径"的地方。

## 已经做的改进

CPU 导入器与呈现管线改成**复用一个** `RDTextureFormat` / `RDTextureView`（建的时候造
...
1. **逐调用点二分**：把 `createStagingTexture`、`cpu_frame_importer.createTexture`、
   `present_pipeline` 的着色器/管线/采样器创建分别摘掉跑一遍，看残留数怎么变。
2. **那 3 个 `RDUniform` 是 refcount 1**：被谁持有还没定位（候选：uniform set 的释放
   被推迟到退出路径）。大概率与上一条同源。
3. 若确认是"引擎在 RID 释放前一直持有描述符"，能做的就是把生命周期**前置**：流停止
   或切换时就释放管线与纹理，而不是等退出。

## 顺带澄清一件事

这一轮播放接线时修掉的那个**退出崩溃**（`nv_ffsw_destroy` → `avcodec_free_context`
段错误）与这里的残留是**两件事**：那是真崩溃（后端被释放两次，根因是调度器的
`unregisterStream` 已经释放过），已修；这里说的是退出时那几行"还有对象没摘干净"的
警告，不影响正确性，但按项目标准属于必须清的账。
